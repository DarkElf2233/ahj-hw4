...
export default function getCardType(cardNumber) {
  for (const cardType in cardTypes) {
    ...
    } else if (cardTypeInfo.lengthRange) {
      if (cardNumber.length >= cardTypeInfo.lengthRange.min
          && cardNumber.length <= cardTypeInfo.lengthRange.max) {
        lengthCheck = true;
      }
    }
    ...
    } else if (cardTypeInfo.prefix && cardNumber.startsWith(cardTypeInfo.prefix)) {
      if (lengthCheck) {
        return cardType;
      }
    }
